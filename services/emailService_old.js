...
/**
 * Creates the HTML and text content for a booking confirmation email.
 * This is a helper function and does not need to be part of the class.
 */
...
class EmailService {
  async sendPoojaBookingConfirmation(bookingData) {
    try {
      // Generate the email content using the helper function from above
  ...
  async sendAdminNotification(bookingData) {
    try {
      ...
      console.error('SendGrid admin notification failed:', error);
      if (error.response) {
        console.error(error.response.body);
      }
  ...
  async sendStoreOrderConfirmation(orderData) {
    try {
      ...
      console.error('SendGrid store order email failed:', error.response?.body || error);
  ...
  async sendStatusUpdateNotification(details) {
    try {
      ...
      console.error('SendGrid status update email failed:', error.response?.body || error);
  ...
  async sendSubscriptionConfirmation(data) {
    try {
      ...
      console.error('SendGrid subscription email sending failed:', error);
      if (error.response) {
        console.error(error.response.body);
      }
      // Do not throw an error, as the payment has already succeeded.
      // Just log it for administrative review.
    }
  }
